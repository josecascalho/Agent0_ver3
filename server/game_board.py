...
class Player(Object):
    ...
    def get_feeling(self,coordinates):
        pass
#------------------------------------------------------------
# CLASS OBSTACLE:
#------------------------------------------------------------
...
class Gift(Object):
    def __init__(self,imageDir,name,x,y,width=0,heigh=0):
        super().__init__(imageDir, name,"gift1","bitmap",x,y,"south")
...
#        self.change_position(object, x, y)
    ...
    #------------------------------------------------
    # REFRESH GIFTS
    def refreshGifts(self,object):
        """Remove the gift that is the agent's coordinates"""
        for obj in self.objects:
            if isinstance(obj,Gift):
                if obj.get_x()==object.get_x() and obj.get_y() == object.get_y():
                    self.remove(obj)
                    return True
        return False
    #--------------------------------------------------
    # ReFRESH FEELINGS
    def refreshFeelings(self,object):
        feelings = []
        for obj in self.objects:
            if isinstance(obj,Gift):
                if obj.get_x() == object.get_x() and obj.get_y() == object.get_y():
                    feelings.append('gift')
            if isinstance(obj, Bomb):
                if (obj.get_x() == object.get_x()-1 and obj.get_y() == object.get_y()) or ( obj.get_x() == object.get_x()+1 and obj.get_y() == object.get_y()) or ( obj.get_x() == object.get_x() and obj.get_y() == object.get_y()+1) or ( obj.get_x() == object.get_x() and obj.get_y() == object.get_y()-1):
                    feelings.append('zz')
                if (obj.get_x() == object.get_x()  and obj.get_y() == object.get_y()):
                    feelings.append('dead')
        feelings.append('unknown')
        return feelings

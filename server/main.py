...
class Server(BaseException):
    ...
    def initialize_gift(self,dirImage,list_gifts,rows,columns):
        i = 1
        for gi in list_gifts:
            gift = gb.Gift(dirImage,'gift1'+str(i),gi[0],gi[1],'gift', False)
            board.add(gift,gi[0],gi[1])
            i = i + 1
                    ...
                    try:
                        ...
                        if type == 'command':
                            ...
                            #-----------------------
                                ...
                                #removing gift after moving on it
                            ...
                            if value == 'feelings':
                                res = board.refreshFeelings(object)
                            if value == 'direction':
                        ...
                        if res != '':
                            ...
                            return_data = str.encode("what? commands= <forward,left,right,set_steps,reset_steps, open_eyes, close_eyes, grab> info=<feelings, direction,view,weights,map,goal,postion,obstacles,maxcoord>")
...
if __name__== "__main__":
    ...
    f.close()
    # Defining the gift ...
    gift_file = './input_files//gift_file.txt'
    f = open(gift_file)
    gift_list = []
    for l in f.readlines():
        if len(l) > 1:
            l = l.split(",")
            gift_list.append((int(l[0]), int(l[1])))
        # print(gift_list)
    server.initialize_gift(images_directory, gift_list, rows, columns)
    f.close()
